...
****/
    ...
    // Keep the input in sync with the real URL, even when it changes dynamically
    function updateInput() {
      input.value = window.location.href;
    }

    // Listen for standard navigation events
    window.addEventListener('popstate', updateInput);
    window.addEventListener('hashchange', updateInput);

    // Monkey-patch history.pushState and history.replaceState to catch SPA navigations
    ...
    window.addEventListener('pushstate', updateInput);
    window.addEventListener('replacestate', updateInput);
