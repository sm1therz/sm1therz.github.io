...
// @version      2.9
// @description  Persistent toggle panel (localStorage-backed). Hashtags: convert # to H2, remove hashtags, capitalize, dashes-to-spaces. Cleanup: remove links, remove == highlights. Notes: clean note blockquotes. Code Blocks: remove bullets (re-indent fenced code). Always-on: list-indentation normalization to tabs. Works with current Workflowy export DOM.
...
#wf-export-toggle-panel .wf-h2.wf-h2-collapsed .wf-h2-collapse{grid-template-rows:0fr}`;
  ...
  // ----------------------------------------------------------------------
  // Code Blocks (TOGGLE: removeCodeBullets)
  // Workflowy exports a code block as a bullet holding the opening fence,
  // then the code lines and closing fence at column 0. When enabled, each
  // block is pulled out to a placeholder BEFORE any other rule runs (so the
  // code body is never touched), and rebuilt at the end. The placeholder
  ...
  function processText(text) {
	...
	// Pull code blocks out first so no other rule (or the tab-normalizer) touches them.
	const extracted = extractCodeBlocks(text, !!state.removeCodeBullets);
